...
class Administrator extends CI_Controller {
	function index(){
	    if($this->session->cek>=3){ //cek login 3 kali
	        $data=array('blokir'=>'Y');
	        $where=array('username'=>$this->input->post('a',TRUE));
	        $this->model_app->update('users',$data,$where);
	        echo $this->session->set_flashdata('message','<div class="alert alert-danger"><center>Akun Terblokir, karena gagal login 3 kali..!!</center></div>');
...
function loginsso($as=null) {
            $tag = "admin_web_inspektorat";
            $tagg = "kontributor_web_inspektorat";
            
            global $extra;

            require_once FCPATH."ssoclient/config.php";
            require_once FCPATH."ssoclient/index.php";

            $appname = "Login Website Inspektorat Purworejo";
            ...
            if(!$sso_client->LoggedIn()) $sso_client->Login("", $appname, $extra);
            
            if($sso_client->HasTag($tag)){
                $user = $sso_client->GetMappedUserInfo(array("email","username","name"));
                $username = $user->username;
                $sql = "SELECT * FROM users where username='".$this->db->escape_str($username)."' AND level='admin' AND blokir='N'";
            ...
            }elseif($sso_client->HasTag($tagg)){
                $user = $sso_client->GetMappedUserInfo(array("email","username","name"));
                $username = $user->username;
                $sql = "SELECT * FROM users where username='".$this->db->escape_str($username)."' AND level='kontributor' AND blokir='N'";
                ...
                $sso_client->Logout();
                header("Location: " . $sso_client->GetFullRequestURLBase());
...
//loginsso end
	...
	function tambah_download(){
		...
		if (isset($_POST['submit'])){
            ...
            $config['max_size'] = '102400'; // kb 100mb
            $this->load->library('upload', $config);
            $this->upload->do_upload('b');
            $hasil=$this->upload->data();
            if ($hasil['file_name']==''){
	...
	function detail_pesanmasuk(){
		...
		if (isset($_POST['submit'])){
            ...
            $this->email->from('inspektorat@purworejokab.go.id', 'INSPEKTORAT PURWOREJO');
            $this->email->to($email);
            $this->email->cc('');
            $this->email->bcc('');
	...
	// Controller Modul Instansi
	function instansi(){
		cek_session_akses('instansi_token',$this->session->id_session);
        if ($this->session->level=='admin'){
            $data['record'] = $this->model_app->view_join_one('instansi_token','instansi','id_inst','id_token','DESC');
        }else{
            $data['record'] = $this->model_app->view_where_ordering('instansi_token',array('username'=>$this->session->username),'id_inst','DESC');
        }
		$this->template->load('administrator/template','administrator/mod_instansi/view_instansi',$data);
	}
    ...
    function tambah_instansi(){
        ...
        if (isset($_POST['submit'])){
            //INSERT INSTANSI
       $instansi=array('instansi'=>$this->db->escape_str($this->input->post('a')));
        $this->model_app->insert('instansi',$instansi);
            //GET ID_INSTANSI
        $inst = $this->db->insert_id();
            //INSERT INSTANSI_TOKEN
           $data = array( 'id_inst' =>$inst,
                'token'=>$this->db->escape_str($this->input->post('b')),
                'batas_token'=>$this->db->escape_str($this->input->post('c')),
                'status'=>$this->db->escape_str($this->input->post('d')));
            $this->model_app->insert('instansi_token',$data);
              

            redirect($this->uri->segment(1).'/instansi');
        }else{
            $this->template->load('administrator/template','administrator/mod_instansi/view_instansi_tambah');
        }
    }
    function edit_instansi(){
		cek_session_akses('instansi',$this->session->id_session);
		$id = $this->uri->segment(3);
		if (isset($_POST['submit'])){
		    $instansi= $this->db->escape_str($this->input->post('a'));
            $data = array( 
                'token'=>$this->db->escape_str($this->input->post('b')),
                'batas_token'=>$this->db->escape_str($this->input->post('c')),
                'status'=>$this->db->escape_str($this->input->post('d')));
            $where = array('id_inst' => $this->input->post('id'));
            $this->model_app->update('instansi_token', $data, $where);
			redirect($this->uri->segment(1).'/instansi');
		}else{
            if ($this->session->level=='admin'){
                 $proses = $this->model_app->edit('instansi_token', array('id_inst' => $id))->row_array();
                 $proses1 = $this->model_app->edit('instansi', array('id_inst' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('instansi_token', array('id_inst' => $id, 'username' => $this->session->username))->row_array();
                $proses1 = $this->model_app->edit('instansi', array('id_inst' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses, 'des' => $proses1);
			$this->template->load('administrator/template','administrator/mod_instansi/view_instansi_edit',$data);
		}
	}
	
	// Controller Modul Kuisioner
		function kuisioner(){
		cek_session_akses('survey_pertanyaan',$this->session->id_session);
        if ($this->session->level=='admin'){
            $data['record'] = $this->model_app->view_ordering('survey_pertanyaan','id_pertanyaan','DESC');
        }else{
            $data['record'] = $this->model_app->view_where_ordering('survey_pertanyaan',array('username'=>$this->session->username),'id_pertanyaan','DESC');
        }
		$this->template->load('administrator/template','administrator/mod_kuisioner/view_kuisioner',$data);
	}
	    function update_deskripsi(){
	    cek_session_akses('survey_deskripsi',$this->session->id_session);
	     if (isset($_POST['submit'])){
            $data = array('deskripsi'=>$this->input->post('des'));
            $where = array('id_deskripsi' => $this->input->post('id'));
            $this->model_app->update('survey_deskripsi', $data, $where);
			redirect($this->uri->segment(1).'/update_deskripsi');
		}else{
		    $proses = $this->model_app->edit('survey_deskripsi', array('id_deskripsi' => 1))->row_array();
		     $data = array ('record' => $proses);
		     $this->template->load('administrator/template','administrator/mod_kuisioner/view_deskripsi_update',$data);
        }
	}
	    function tambah_kuisioner(){
        cek_session_akses('survey_pertanyaan',$this->session->id_session);
        //CEK PILIHAN
        $pilihan = $this->db->escape_str($this->input->post('d'));
        if($pilihan==0){
            $pilih=null;
        }else{
            $pilih=1;
        ...
        if (isset($_POST['submit'])){
           $data = array('tahun'=>$this->db->escape_str($this->input->post('a')),
                'semester'=>$this->db->escape_str($this->input->post('b')),
                'pertanyaan'=>$this->input->post('c'),
                'id_group_pilihan'=>$pilih,
                'status'=>$this->db->escape_str($this->input->post('e')));
            $this->model_app->insert('survey_pertanyaan',$data);  
            redirect($this->uri->segment(1).'/kuisioner');
        }else{
            $data['record'] = $this->model_app->view_ordering('survey_pertanyaan','id_pertanyaan','DESC');
            $this->template->load('administrator/template','administrator/mod_kuisioner/view_kuisioner_tambah',$data);
        }
    }
    ...
    function edit_kuisioner(){
		...
		if (isset($_POST['submit'])){
            $data = array('tahun'=>$this->db->escape_str($this->input->post('a')),
                ...
                'id_group_pilihan'=>$pilih,
                'status'=>$this->db->escape_str($this->input->post('e')));
            $where = array('id_pertanyaan' => $this->input->post('id'));
            $this->model_app->update('survey_pertanyaan', $data, $where);
			redirect($this->uri->segment(1).'/kuisioner');
		}else{
            if ($this->session->level=='admin'){
                 $proses = $this->model_app->edit('survey_pertanyaan', array('id_pertanyaan' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('survey_pertanyaan', array('id_pertanyaan' => $id, 'username' => $this->session->username))->row_array();
            }
            $datas['rows'] = $proses; 
            //$datas['record'] = $this->model_app->view_ordering('aspek','id_aspek','DESC');
			$this->template->load('administrator/template','administrator/mod_kuisioner/view_kuisioner_edit',$datas);
		}
	}
	...
	function delete_kuisioner(){
        cek_session_akses('survey_pertanyaan',$this->session->id_session);
        if ($this->session->level=='admin'){
    		$id = array('id_pertanyaan' => $this->uri->segment(3));
        }else{
            $id = array('id_pertanyaan' => $this->uri->segment(3), 'username'=>$this->session->username);
        }
		$this->model_app->delete('survey_pertanyaan',$id);
		redirect($this->uri->segment(1).'/kuisioner');
    }
    
   /* function update_open_deskripsi(){
        cek_session_akses('survey_deskripsi',$this->session->id_session);
        $id = $this->uri->segment(3);
		if (isset($_POST['submit'])){
            $data = array('deskripsi'=>$this->input->post('description'));
            $where = array('id_deskripsi' => $this->input->post('id'));
            $this->model_app->update('survey_deskripsi', $data, $where);
			redirect($this->uri->segment(1).'/kuisioner');
		}else{
            if ($this->session->level=='admin'){
                 $proses = $this->model_app->edit('survey_deskripsi', array('id_deskripsi' => $id))->row_array();
            }else{
                $proses = $this->model_app->edit('survey_deskripsi', array('id_deskripsi' => $id, 'username' => $this->session->username))->row_array();
            }
            $datas['rows'] = $proses; 
			$this->template->load('administrator/template','administrator/mod_kuisioner/view_kuisioner',$datas);
		}
    } */
	
		// Controller Modul Hasil Laporan Kuisioner
	function laporan(){
		cek_session_akses('survey_responden',$this->session->id_session);
        if ($this->session->level=='admin'){
            $data['record'] = $this->model_app->view_ordering('survey_responden','id_responden','DESC');
        }else{
            $data['record'] = $this->model_app->view_where_ordering('survey_responden',array('username'=>$this->session->username),'id_responden','DESC');
        }
		$this->template->load('administrator/template','administrator/mod_laporan/view_laporan',$data);
	}
	
	 function tambah_aspek(){
        //cek_session_aspek('aspek',$this->session->id_session);
        if (isset($_POST['submit'])){
           $data = array('no_urut'=>$this->db->escape_str($this->input->post('a')),
                'jdl_aspek'=>$this->db->escape_str($this->input->post('b')));
            $this->model_app->insert('aspek',$data);  
            redirect($this->uri->segment(1).'/aspek');
        }else{
            $data['record'] = $this->model_app->view_ordering('aspek','id_aspek','DESC');
            $this->template->load('administrator/template','administrator/mod_aspek/view_aspek_tambah',$data);
        }
    }
}
